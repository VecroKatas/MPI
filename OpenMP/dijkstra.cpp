...
#pragma omp parallel for default(none) shared(distances, prevNodes, weights, currentNode, unvisited, nodesCount) 
        for (int node = 0; node < nodesCount; ++node) {
            if (unvisited.find(node) == unvisited.end()) continue;

            if (weights[currentNode][node] != -1) {
                int nodeDistance = weights[currentNode][node];
...
#pragma omp parallel for default(none) shared(distances, unvisited, nodesCount) reduction(min:minCost)
        for (int node = 0; node < nodesCount; ++node) {
